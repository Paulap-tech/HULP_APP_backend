...
app.post('/registro', (req, res) => {
    ...
    db.query('SELECT * FROM usuarios WHERE email = ?', [email], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            db.query('INSERT INTO usuarios (email, password, age, ano_diagnostico, ano_sintomas) VALUES (?, ?, ?, ?, ?)', [email, password, age, ano_diagnostico,ano_sintomas], (err, result) => {
                if (err) {
...
app.post('/api/ciclo-duracion', (req, res) => {
    ...
    const query = 'UPDATE usuarios SET cicle_days = ? WHERE email = ?';
...
app.post ('/information', (req, res) => {
    ...
    const query = 'UPDATE usuarios SET talla = ?, tratamiento = ?, peso = ? WHERE email = ?';
...
app.get('/ciclo/:email', (req, res) => {
    ...
    const query = 'SELECT cicle_days FROM usuarios WHERE email = ?';
...
app.get('/registros/fechas/:email', (req, res) => {
    ...
    const query = 'SELECT fecha_inicio, fecha_fin, fase FROM menstruacion WHERE email = ?';
    db.query(query, [email], (err, results) => {
        if (err) {
...
app.post('/menstruacion', (req, res) => {
    ...
    const query = 'INSERT INTO menstruacion (email, fecha_inicio, fecha_fin, fase) VALUES (?, ?, ?, ?)';
...
app.get('/fase-menstrual/:email/:date', (req, res) => {
    ...
    const query = 'SELECT fase FROM menstruacion WHERE email = ? AND fecha_inicio <= ? AND fecha_fin >= ?';
    db.query(query, [email, date, date], (err, results) => {
        ...
        res.status(200).json({ fase: results[0].fase });
    });
});
...
app.post('/registros', (req, res) => {
    ...
    const getFaseQuery = 'SELECT fase FROM menstruacion WHERE email = ? AND fecha_inicio <= ? AND fecha_fin >= ?';
    db.query(getFaseQuery, [email, fecha, fecha], (err, results) => {
        ...
        const faseMenstrual = results.length > 0 ? results[0].fase : null;

        // Ahora guardamos el registro con la fase menstrual
        const insertQuery = 'INSERT INTO registros (date, faseMenstrual, cataplejiaExtremidades, cataplejiaFacial, cataplejiaSuelo, suenoFragmentado, paralisisSuenio, somnolencia, email) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
